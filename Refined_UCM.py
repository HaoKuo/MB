...
def call_samtools_sortN(prefix):
    pcmd = "samtools sort -n "+ prefix + ".temp.bam -o " + prefix + ".temp.sort.bam"
    command_line = ["/bin/bash", "-c", pcmd]
    subprocess.check_call(command_line)


def make_temp_sorted_bam(in_bam,prefix,tag_len,spcr_len):
    ...
    for line in in_bam_file.fetch(until_eof=True):
        ...
        if paired_end_count % 2 == 0:
            ...
            if temp_read1_entry.query_sequence[:tag_len] > line.query_alignment_sequence[:tag_len]:
                temp_bam_entry.query_name = temp_read1_entry.query_sequence[:tag_len] + \
                                        line.query_alignment_sequence[:tag_len] + '#ab'

            elif temp_read1_entry.query_sequence[:tag_len] < line.query_alignment_sequence[:tag_len]:
                temp_bam_entry.query_name = line.query_alignment_sequence[:tag_len] + \
                                            temp_read1_entry.query_sequence[:tag_len] + '#ba'
...
def main():
    ...
    #make_temp_sorted_bam(o.in_bam,o.prefix,o.tag_len,o.spcr_len)
