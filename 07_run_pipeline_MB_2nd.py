...
fastq_dir = "/work-z/user/guoh/tech-RD/MB4/SSCS_fq/"
#fastq_info = fastq_dir+"fastq_files.txt.gz"

tmp_dir = work_dir+"tmp/"
mapped_dir = work_dir + "SSCS_mapped2/"
#fastQC_dir = work_dir + "fastqc/"
#guesseq_dir = work_dir + "guesseq/"
#fastqToBam_dir = work_dir + "fastqToBam/"
fragment_dir = work_dir + "SSCS_fragment/"
mpileup_all_dir = work_dir + "SSCS_mpileup_all/"
pyvacheck_dir = work_dir + "SSCS_pyvacheck/"
varscan_dir = work_dir + "SSCS_varscan/"


for d in [tmp_dir, mapped_dir, mpileup_all_dir, varscan_dir, pyvacheck_dir, fragment_dir]:
...
def main():
    ...
    sample_name = ["I"+str(s) for s in xrange(1,7)]+\
            ["I"+str(ss) for ss in xrange(26,32)]
    status = []
    for name in sample_name:
        ...
        print lane, i 
        R1 = fastq_dir+i+"_read1_sscs.fq.gz"
        R2 = fastq_dir+i+"_read2_sscs.fq.gz"
...
def call_BwaMem(R1,R2,lane,name):
    ...
    cmd3 = "|"+SAMTOOLS+" view -Sb - > SSCS_mapped2/"+lane+"_"+name+".bam"
    return cmd1+cmd2+cmd3
